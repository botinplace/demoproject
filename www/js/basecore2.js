...
var ButtonsEventListener = function (e) {
    ...
    // Сначала добавляем файлы, если они есть
    const fileField = this.querySelector('input[type="file"]');
    if (fileField && fileField.files.length > 0) {
        for (let i = 0; i < fileField.files.length; i++) {
            formData.append('afile[]', fileField.files[i]);
        }
        // Добавляем атрибут data-action, если он существует
        if (fileField.getAttribute('data-action')) {
            formData.append(fileField.getAttribute('data-action'), true);
        }
    }

    // Затем добавляем остальные данные формы
    const hobbies = this.querySelectorAll('input[name="hobbies[]"]');
    let hobbies_str = '';
    if (hobbies) {
        for (let i = 0; i < hobbies.length; i++) {
            if (hobbies[i].checked) {
                hobbies_str += (hobbies_str ? ',' : '') + hobbies[i].value; // Создаем строку из выбранных хобби
            }
        }
    }

    if (hobbies_str) {
        formData.append('hobbies_str', hobbies_str); // Добавляем строку хобби к FormData
    }

    // Добавляем все остальные поля формы
    const otherFields = this.elements;
    for (let field of otherFields) {
        if (field.name && field.type !== 'file' && field.type !== 'checkbox') {
            formData.append(field.name, field.value);
        }
    }

    const url = this.getAttribute('action') ? this.getAttribute('action') : window.location.href;
    const form_method = this.getAttribute('method') ? this.getAttribute('method') : 'GET';
    ajaxSend(formData, url, button, form_method);
};
...
var ajaxSend = (formData, form_url, btn, method) => {
    const url = new URL(form_url, window.location.origin);
    let headers_content = {
        'credentials': 'same-origin',
        'X-Requested-With': 'XMLHttpRequest',
    };

    // Проверка, есть ли файлы для установки Content-Type
    if (!formData.has('afile[]')) {
        headers_content['Content-Type'] = 'application/json'; // Размещаем Content-Type только если файлы отсутствуют
    }

    // Определяем параметры fetch
    const fetch_params = {
        method: method,
    ...
    fetch(url, fetch_params)
        ...
        .then(response => {
            if (response.success) {
                ...
                } else if (response.action === 'function') {
                    const functionName = response.fname.toString();
                    const respFunc = window[functionName];
                    if (typeof respFunc === "function") {
                        respFunc(response.fvalue || undefined);
                    }
                } else {
                ...
                setTimeout(() => { btn.classList.remove('loading_b'); }, 500);
            } else {
        ...
        .catch((error) => {
            console.error(error);
            show_info('Ошибка', true);
            setTimeout(() => { btn.classList.remove('loading_b'); }, 500);
        });
};

// Функция для обработки ошибок
const handleErrorResponse = (response, btn) => {
    btn.classList.remove('loading_b');
    if (document.querySelector('.error-info')) {
        document.querySelector('.error-info').innerHTML = response.error_text || 'Неизвестная ошибка';
